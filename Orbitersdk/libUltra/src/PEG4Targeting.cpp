...
bool PEG4Targeting::Step()
{
	if(propagatorStepsRemaining > 0) {
		...
		if(currentState == BURN_PROP) {
			...
			char cbuf[255];
			sprintf(cbuf, "Miss offset: %f %f %f Time: %f", missOffset.x, missOffset.y, missOffset.z, coastTime+burnTime);
...
void PEG4Targeting::PerformTargetingIteration()
{
	...
	char cbuf[255];
	sprintf(cbuf, "DV: %f %f %f", equDeltaV.x, equDeltaV.y, equDeltaV.z);
	oapiWriteLog(cbuf);
